	...
	return C.generate = function (a) {
		...
		for (o = r.axis_rotated ? function (a) {
				...
				var h, i = a.x.getTime(),
					...
					l = new Date(i + k * f),
					m = new Date(i + k * (f + g));
				return h = r.axis_rotated ? [[c(j(f)), b(l)], [c(j(f + g)), b(m)]] : [[b(l), c(j(f))], [b(m), c(j(f + g))]], e(h)
			} : function (a, d, f, g) {
				var h;
				return h = r.axis_rotated ? [[c(j(f), !0), b(i(f))], [c(j(f + g), !0), b(i(f + g))]] : [[b(i(f), !0), c(j(f))], [b(i(f + g), !0), c(j(f + g))]], e(h)
			}, f = 0; f < a.length; f++) {
...
function () {
	...
	var ph = hg.locale({
		...
		months: ["January", "فوریه", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"],
	...
	hg.geo.clipExtent = function () {
		...
		return a.invert = function (a) {
			...
			var j = f.scale(),
				k = +b[0],
				l = +b[1];
			return c = f.translate(b).clipExtent([[k - .455 * j, l - .238 * j], [k + .455 * j, l + .238 * j]]).stream(i).point, d = g.translate([k - .307 * j, l + .201 * j]).clipExtent([[k - .425 * j + Ig, l + .12 * j + Ig], [k - .214 * j - Ig, l + .234 * j - Ig]]).stream(i).point, e = h.translate([k - .205 * j, l + .212 * j]).clipExtent([[k - .214 * j + Ig, l + .166 * j + Ig], [k - .115 * j - Ig, l + .234 * j - Ig]]).stream(i).point, a
		}, a.scale(1070)
	};
	...
	hg.geo.path = function () {
		...
		return a.area = function (a) {
			...
			return Oh = Ph = -(Mh = Nh = 1 / 0), hg.geo.stream(a, e(Rh)), [[Mh, Nh], [Oh, Ph]]
		...
		}, a.majorExtent([[-180, -90 + Ig], [180, 90 - Ig]]).minorExtent([[-180, -80 - Ig], [180, 80 + Ig]])
	...
	Qc.prototype.prepare = function () {
		...
		function b(a) {
			...
			return gd(c(a), h).cells.forEach(function (c, h) {
				...
				(b[h] = i.length ? i.map(function (a) {
					var b = a.start();
					return [b.x, b.y]
				}) : j.x >= d && j.x <= f && j.y >= e && j.y <= g ? [[d, g], [f, g], [f, e], [d, e]] : []).point = a[h]
			}), b
		}
		...
		return a ? b(a) : (b.links = function (a) {
			...
			return arguments.length ? b.clipExtent(a && [[0, 0], a]) : h === gi ? null : h && h[1]
		}, b)
	};
	var gi = [[-1e6, -1e6], [1e6, 1e6]];
	...
	var Qi = {
		...
		Ri = [["n", "e", "s", "w", "nw", "ne", "se", "sw"], ["e", "w"], ["n", "s"], []],
	...
	var Vi = [1e3, 5e3, 15e3, 3e4, 6e4, 3e5, 9e5, 18e5, 36e5, 108e5, 216e5, 432e5, 864e5, 1728e5, 6048e5, 2592e6, 7776e6, 31536e6],
		Wi = [[jh.second, 1], [jh.second, 5], [jh.second, 15], [jh.second, 30], [jh.minute, 1], [jh.minute, 5], [jh.minute, 15], [jh.minute, 30], [jh.hour, 1], [jh.hour, 3], [jh.hour, 6], [jh.hour, 12], [jh.day, 1], [jh.day, 2], [jh.week, 1], [jh.month, 1], [jh.month, 3], [jh.year, 1]],
		Xi = Si.multi([[".%L", function (a) {
			return a.getMilliseconds()
		}], [":%S", function (a) {
			return a.getSeconds()
		}], ["%I:%M", function (a) {
